...
from control_action_client import ControlActionClient

class ControlGUI:
    def __init__(self, root, action_client):
    ...
    def button_action(self, control):
        ...
        self.action_client.send_goal(control)
...
def main(args=None):
    ...
    # Create the ROS 2 action client node
    control_action_client = ControlActionClient()

    # Start the ROS 2 thread
    ros2_thread = ROS2Thread(control_action_client)
    ros2_thread.start()

    # Start the Tkinter GUI
    root = tk.Tk()
    gui = ControlGUI(root, control_action_client)
    root.mainloop()

    # Shutdown ROS 2
    control_action_client.destroy_node()
    rclpy.shutdown()
